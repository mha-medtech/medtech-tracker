![Version](https://img.shields.io/badge/version-1.4.1-blue)
![Last Updated](https://img.shields.io/badge/updated-June%202026-green)
![Languages](https://img.shields.io/badge/languages-EN%20%7C%20DE%20%7C%20FA-orange)
![Status](https://img.shields.io/badge/status-active-brightgreen)

# MedTech Tracker 🏥

A web-based medical equipment tracking system designed for clinics and hospitals.
Built by a Biomedical Engineer with real-world experience in medical device regulation and management.

## 🌐 Live Demo
👉 [medtracker.freedev.app](https://medtracker.freedev.app) — Full hosted version  
👉 [mha-medtech.github.io/medtech-tracker](https://mha-medtech.github.io/medtech-tracker) — GitHub Pages version

---

## ✨ Features
- 📊 Real-time dashboard with equipment statistics
- 📈 Interactive doughnut chart (Chart.js)
- ⚠️ Calibration alert system — overdue and upcoming warnings
- ➕ Add, edit, and delete equipment records
- 🔍 Search and filter by status
- 🌙 Dark / Light mode toggle
- 🌍 Multilingual support (English / Deutsch / فارسی)
- 📱 Fully responsive — mobile & tablet friendly
- 💾 Data persistence via localStorage

---

## 🛠️ Tech Stack
- Frontend: HTML5, CSS3, JavaScript (Vanilla)
- Charts: Chart.js
- Backend *(coming soon)*: PHP
- Database *(coming soon)*: MySQL
- Deployment: GitHub Pages → Railway (planned)

---

## 🗺️ Roadmap
- [x] Equipment CRUD (Add / Edit / Delete)
- [x] Calibration alert system
- [x] Multilingual (EN / DE / FA)
- [x] Dark mode
- [x] Responsive design
- [x] Export to CSV
- [ ] User authentication & login
- [ ] PHP + MySQL backend
- [ ] Deploy on live server

---

## 👨‍💻 Developer
**Mohammad Hassan Abbasi**
Biomedical Engineer | CEO at Kayazh | MSc Applicant

- 🌐 [kayazh.com](https://www.kayazh.com)
- 💼 [LinkedIn](https://www.linkedin.com/in/mhabbasii/)
- 🐙 [GitHub](https://github.com/mha-medtech)

---

## 📄 License
© 2026 Mohammad Hassan Abbasi. All rights reserved.